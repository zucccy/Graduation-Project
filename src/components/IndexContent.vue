              ...
              <ul>
                <li v-for="(item,index) in officeList" :key="index">
                  <span class="title">{{item.officeName}}</span>
                    <a @click="toDoctorInfo(child.id)" v-for="(child,index) in item.childOfficeList" :key="index">{{child.officeName}}</a>
                </li>
              </ul>
            </div>
          </section>
          <div class="mainframe">
            <section class="block tab">
              <div class="title">
                ...
                <el-menu :default-active="activeIndex_menu1" class="el-menu-demo"
                         mode="horizontal" style="display: inline-block"
                         @select="handleSelect_menu1" active-text-color="#74bc31"
                >
              ...
              <div class="tabcontent" v-show="activeIndex_menu1==1">
                <div class="single" v-for="(item,index) in hospitalList" :key="index">
                  <img :src="item.src" alt="">
                  <b class="about" @click="toHospitalInfo(item.id)" >{{item.hospitalName}}</b>
          ...
          <div class="mainframe">
            <section class="block tab">
              <div class="title">
                ...
                <el-menu :default-active="activeIndex_menu2" class="el-menu-demo"
                         mode="horizontal" style="display: inline-block"
                         @select="handleSelect_menu2" active-text-color="#74bc31"
                >
                  <el-menu-item index="1">内科</el-menu-item>
                  <el-menu-item index="2">外科</el-menu-item>
                  <el-menu-item index="3">妇产科</el-menu-item>
              ...
              <div class="tabcontent" v-show="activeIndex_menu2==1">
                <div class="person" v-for="(item,index) in doctorVOList" :key="index">
                  <a href="" @click="toDoctorAppointment(item.id, item.officeId, item.hospitalId)">
                    <img :src="item.src" alt="">
                    <strong><b>{{item.doctorName}}</b></strong>
                    <p>{{item.position}}</p>
          ...
          <section class="block tab">
            <section class="smallblock">
              <div class="title">
                ...
                <el-menu :default-active="activeIndex_menu3" class="el-menu-demo"
                         mode="horizontal" style="display: inline-block"
                         @select="handleSelect_menu3" active-text-color="#74bc31"
                >
                  <el-menu-item index="1">内科</el-menu-item>
                  <el-menu-item index="2">外科</el-menu-item>
              ...
              <div class="tabcontent" v-show="activeIndex_menu3==1">
                <div class="single" v-for="item in hospitalList" :key="item.name">
                  <strong>XXXX</strong>
                  <p>
                    <a href=""><img class="pic" src="" alt="" ></a>
                    <b>XXXXX</b>
                    XXXXX
                  </p>
                </div>
    ...
    export default {
        ...
        data() {
          return {
            ...
            department:"",
            channellist:[
              "肝病","不孕不育","腰椎间盘突出","颈椎病","胆结石","鼻炎","白癜风","甲亢","肾结石","性病","抑郁症","癫痫",
              "胃病","头疼","口臭","便秘","狐臭","皮肤病",
            ],
            officeList:[],
            hospitalList:[],
      ...
      methods: {
        toDoctorInfo(val){
          this.$router.push({path: 'DoctorInfo', query: {
            id: val,
            status: 'doctor',
            }
          })
        ...
        toHospitalInfo(val){
          this.$router.push({path: 'HospitalInfo', query: {
            id: val,
            }
          })
        },
        ...
        getNewsList(){
          var param = {};
          this.$axios.get("/hospital/getHospitalNews",param).then((res) => {
            if (200 == res.status){
              console.log(res)
        ...
        getHospitalList(){
          ...
          this.$axios.post("/hospital/getAdviceHospitalList",param).then((res) => {
            if (200 == res.status){
              console.log(res)
        ...
        getAllOfficeList(){
          // var param = {id:1};
          this.$axios.get("/office/getOfficeNameAll",{
            params:{
            }
          }).then((res) => {
            if (200 == res.status){
...
<style scoped>
  ...
  .title a {
    ...
    background: url(https://image.39.net/dist/yyk/v3/pc/images/arrow_more.png) right center no-repeat;
  }
  .content {
    display: inline-block;
    width: 322px;
